  ...
  try {
    ...
    val f = setupFuture
      ...
      .flatMap { _ =>
        ...
        println(
          "Generated SQL for filter query:\n" + filterQuery.result.statements
        )
      ...
      .flatMap { _ =>
        ...
        println(
          "Generated SQL for Coffees update:\n" + updateQuery.updateStatement
        )
        println(
          "Generated SQL for Coffees update:\n" + updateQuery.insertStatement
        )
      ...
      .flatMap { _ =>
        ...
        val joinQuery
            : Query[(Rep[String], Rep[String]), (String, String), Seq] = for {
          c <- coffees if c.price > 9.0
          s <- c.supplier
        } yield (c.name, s.name)

        println(
          "Generated SQL for the join query:\n" + joinQuery.result.statements
        )
      ...
      .flatMap { _ =>
        ...
        println(
          "Generated SQL for max price column:\n" + maxPrice.result.statements
        )
      ...
      .flatMap { _ =>
        ...
        val plainQuery =
          sql"select SUP_NAME from SUPPLIERS where STATE = $state".as[String]
        // #plainSql
